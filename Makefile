...
# parameters
REGISTRYSERVER=services-registry.cloudapp-enterprise-preprod.appctest.com:5000
REGISTRYPROJECTNAME=k8sdev
DEVFLAG=true
NOTARYFLAG=false
CLAIRFLAG=false
HTTPPROXY=
...
compile_core:
	...
	@echo GOIMAGEBUILDCMD=$(GOIMAGEBUILDCMD)
	@echo GOFLAGS=$(GOFLAGS)
	@echo GOTAGS=$(GOTAGS)
	@echo CORE_LDFLAGS=$(CORE_LDFLAGS)

	@echo BUILDPATH=$(BUILDPATH)
	@echo GOBUILDPATH=$(GOBUILDPATH)
	@echo GOBUILDPATH_CORE=$(GOBUILDPATH_CORE)
	@echo GOBUILDIMAGE=$(GOBUILDIMAGE)
	@echo GOIMAGEBUILD_CORE=$(GOIMAGEBUILD_CORE)
	@echo GOBUILDMAKEPATH_CORE=$(GOBUILDMAKEPATH_CORE)
	@echo CORE_BINARYNAME=$(CORE_BINARYNAME)
...
# ars custom target
build_jobservice:
	make -f $(MAKEFILEPATH_PHOTON)/Makefile _build_jobservice -e DEVFLAG=$(DEVFLAG) -e GOBUILDIMAGE=$(GOBUILDIMAGE) \
	 -e REGISTRYVERSION=$(REGISTRYVERSION) -e REGISTRY_SRC_TAG=$(REGISTRY_SRC_TAG) -e NGINXVERSION=$(NGINXVERSION) -e NOTARYVERSION=$(NOTARYVERSION) -e NOTARYMIGRATEVERSION=$(NOTARYMIGRATEVERSION) \
	 -e CLAIRVERSION=$(CLAIRVERSION) -e CLAIRADAPTERVERSION=$(CLAIRADAPTERVERSION) -e VERSIONTAG=$(VERSIONTAG) \
	 -e BUILDBIN=$(BUILDBIN) -e REDISVERSION=$(REDISVERSION) -e MIGRATORVERSION=$(MIGRATORVERSION) \
	 -e CHARTMUSEUMVERSION=$(CHARTMUSEUMVERSION) -e DOCKERIMAGENAME_CHART_SERVER=$(DOCKERIMAGENAME_CHART_SERVER) \
...
# ars custom target
build_core:
	make -f $(MAKEFILEPATH_PHOTON)/Makefile _build_core -e DEVFLAG=$(DEVFLAG) -e GOBUILDIMAGE=$(GOBUILDIMAGE) \
	 -e REGISTRYVERSION=$(REGISTRYVERSION) -e REGISTRY_SRC_TAG=$(REGISTRY_SRC_TAG) -e NGINXVERSION=$(NGINXVERSION) -e NOTARYVERSION=$(NOTARYVERSION) -e NOTARYMIGRATEVERSION=$(NOTARYMIGRATEVERSION) \
	 -e CLAIRVERSION=$(CLAIRVERSION) -e CLAIRADAPTERVERSION=$(CLAIRADAPTERVERSION) -e VERSIONTAG=$(VERSIONTAG) \
	 -e BUILDBIN=$(BUILDBIN) -e REDISVERSION=$(REDISVERSION) -e MIGRATORVERSION=$(MIGRATORVERSION) \
	 -e CHARTMUSEUMVERSION=$(CHARTMUSEUMVERSION) -e DOCKERIMAGENAME_CHART_SERVER=$(DOCKERIMAGENAME_CHART_SERVER) \
...
# ars custom target
build_base_docker_jobservice:
	@for name in jobservice; do \
		echo $$name ; \
		$(DOCKERBUILD) --pull -f $(MAKEFILEPATH_PHOTON)/$$name/Dockerfile.base -t goharbor/harbor-$$name-base:$(BASEIMAGETAG) . ; \
	done
...
# ars custom target
pushimage_core:
	@echo "pushing harbor-core image ..."
	$(DOCKERTAG) $(DOCKERIMAGENAME_CORE):$(VERSIONTAG) $(REGISTRYSERVER)/$(REGISTRYPROJECTNAME)/harbor-core:$(VERSIONTAG)
	@$(PUSHSCRIPTPATH)/$(PUSHSCRIPTNAME) $(REGISTRYSERVER)/$(REGISTRYPROJECTNAME)/harbor-core:$(VERSIONTAG) \
		$(REGISTRYUSER) $(REGISTRYPASSWORD) $(REGISTRYSERVER)
	@$(DOCKERRMIMAGE) $(REGISTRYSERVER)$(DOCKERIMAGENAME_CORE):$(VERSIONTAG)

# ars custom target
pushimage_jobservice:
	@echo "pushing harbor-jobservice image ..."
	$(DOCKERTAG) $(DOCKERIMAGENAME_JOBSERVICE):$(VERSIONTAG) $(REGISTRYSERVER)/$(REGISTRYPROJECTNAME)/harbor-jobservice:$(VERSIONTAG)
	@$(PUSHSCRIPTPATH)/$(PUSHSCRIPTNAME) $(REGISTRYSERVER)/$(REGISTRYPROJECTNAME)/harbor-jobservice:$(VERSIONTAG) \
		$(REGISTRYUSER) $(REGISTRYPASSWORD) $(REGISTRYSERVER)
	@$(DOCKERRMIMAGE) $(REGISTRYSERVER)$(DOCKERIMAGENAME_JOBSERVICE):$(VERSIONTAG)

ship_core: compile_core build_base_docker_core build_core pushimage_core
